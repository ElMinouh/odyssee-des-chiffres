...
// v12.4.41 (retour Cyril, point 1 : vérification approfondie a trouvé un vrai
// doublon exact entre 2 zones château — "La Tour des Verbes" / "Le Beffroi des
// Conjugaisons") : pools élargis à 8 particules (au lieu de 6) pour réduire
// drastiquement les collisions par pur hasard, ET nombre d'éléments affichés
// désormais variable (3 à 5 selon la zone, voir _zoneDecorFor), ce qui
// multiplie encore l'espace de combinaisons possibles par famille.
const _ZONE_DECOR_CATALOG = {
 ...
 romain:      { kw:['forum','thermes','circus','voie appienne','ruines latines','cicéron','tribune','agora','amphithéâtre','temple'], particles:['🏛️','🕊️','☀️','🏺','📜','⚔️','🗿','🏟️'], rel:['egypte','theatre'] },
 egypte:      { kw:['gizeh','nil','thèbes','pyramide','tombeau'], particles:['🐫','☀️','🏺','📜','⏳','🦂','🏜️','⚱️'], rel:['romain','desert'] },
 prehistoire: { kw:['mammouth','chasseur','peinture','foyer','tambour'], particles:['🔥','🦴','🪨','🦣','🏹','🌑','🌋','🐾'], rel:['foret','montagne'] },
 moyenage:    { kw:['médiéval','fief','veillée','assiégé','chancelier','gaulois'], particles:['🕯️','⚔️','🏰','🛡️','🚩','🔔','🗡️','📯'], rel:['chateau'] },
 theatre:     { kw:['théâtre','scène','masque','voix'], particles:['🎭','✨','🎪','🎬','🎟️','👑','🎫','🕺'], rel:['romain'] },
 mecanique:   { kw:['mécanique','engrenage','rouage','horlogerie','horloge','machine','atelier','usine','forge','révolution','vapeur','gare','invention','inventeur'], particles:['⚙️','✨','🔧','🔩','💡','🛠️','⏱️','🔌'], rel:['abstrait'] },
 ...
 ile:         { kw:['île','tortue'], particles:['🐢','🌴','🐚','🌊','🦜','☀️','🥥','🏝️'], rel:['plage','ocean-profond'] },
 glace:       { kw:['glace','glacier','banquise'], particles:['❄️','🌨️','💎','🧊','⛄','✨','🐧','🏔️'], rel:['montagne'] },
 grotte:      { kw:['grotte','caverne','souterrain','cavernes','antre'], particles:['💎','🦇','✨','🪨','🕳️','💠','🕸️','👻'], rel:['montagne','foret'] },
 champignon:  { kw:['champignon'], particles:['🍄','🐌','✨','🌿','🦋','🍂','🌰','🐛'], rel:['foret'] },
 pont:        { kw:['pont'], particles:['🌉','🕊️','✨','💧','🍃','🌊','🚶','🏞️'], rel:['ruisseau','village'] },
 volcan:      { kw:['volcan','feu','flamme','lave','trône de l'], particles:['🔥','💨','⚫','🌋','✨','🪨','🌡️','🗻'], rel:['desert','montagne'] },
 desert:      { kw:['désert','cendre','caldeira'], particles:['🏜️','☀️','💨','🦂','🌵','🐫','🐪','⏳'], rel:['volcan','montagne'] },
 montagne:    { kw:['pic','crête','cime','sommet','plateau','falaise','gorge','vallée'], particles:['🦅','☁️','⛰️','🌬️','🪨','✨','🥾','🌤️'], rel:['desert','glace'] },
 ...
 langue:      { kw:['mot','langue','syntaxe','synonyme','nom','lettre','verbe','phrase','conjugaison','registre'], particles:['📝','✨','🔤','📖','🖋️','💬','📔','✏️'], rel:['bibliotheque','abstrait'] },
 ...
 observatoire:{ kw:['observatoire','étoile','nuage','ciel','céleste','stellaire','cerf-volant'], particles:['⭐','☁️','✨','🔭','🌙','💫','🛰️','✴️'], rel:['nuit','espace'] },
 nuit:        { kw:['nocturne','ombre','lune'], particles:['🌙','⭐','🦇','✨','🌌','🦉','🌃','🐺'], rel:['observatoire','espace'] },
 espace:      { kw:['galaxie','espace','nébuleuse','cosmique'], particles:['💫','🪐','🌠','⭐','🌌','🚀','🛰️','🌍'], rel:['observatoire','nuit'] },
};
...
function _zoneDecorFor(zoneId, label){
 ...
 // Tire 3 à 5 éléments distincts du pool élargi (compte variable = combos
 // encore plus nombreux), ordre mélangé.
 const chosen = [];
 const drawCount = 3 + Math.floor(_archHash(seed, 5099) * 3); // 3, 4 ou 5
...
const _ISLAND_PROFILE_VARIANTS = {
 colline:    [
  ...
  // v12.4.44 (Lot 3, dette technique v19) — variantes 3 à 6, validées sur
  // maquette (maquette-formes-ilots.html) avant implémentation.
  [1.1, 1.17, 1.18, 1.14, 1.11, 1.05, 0.94, 0.87, 0.86, 0.9, 0.93, 0.99],
  [0.88, 0.97, 1.07, 1.13, 1.15, 1.18, 1.16, 1.07, 0.97, 0.91, 0.89, 0.86],
  [0.89, 0.85, 0.88, 0.96, 1.02, 1.08, 1.15, 1.2, 1.16, 1.08, 1.02, 0.96],
  [1.08, 0.99, 0.89, 0.86, 0.88, 0.91, 0.96, 1.05, 1.15, 1.18, 1.16, 1.13],
 ],
 feuille:    [
  ...
  [1.13, 1.19, 1.2, 1.16, 1.07, 0.97, 0.87, 0.85, 0.85, 0.85, 0.93, 1.03],
  [0.87, 0.99, 1.11, 1.2, 1.22, 1.21, 1.13, 1.01, 0.89, 0.85, 0.85, 0.85],
  [0.85, 0.85, 0.85, 0.95, 1.07, 1.18, 1.22, 1.22, 1.16, 1.05, 0.93, 0.85],
  [0.98, 0.87, 0.85, 0.85, 0.85, 0.91, 1.02, 1.13, 1.21, 1.22, 1.18, 1.09],
 ],
 dune:       [
  ...
  [1.14, 1.13, 1.03, 0.92, 0.93, 1.03, 1.14, 1.13, 1.03, 0.92, 0.93, 1.03],
  [0.96, 1.13, 1.1, 0.93, 0.96, 1.13, 1.1, 0.93, 0.96, 1.13, 1.1, 0.93],
  [0.95, 0.91, 0.99, 1.11, 1.15, 1.07, 0.95, 0.91, 0.99, 1.11, 1.15, 1.07],
  [1.05, 1.15, 1.01, 0.91, 1.05, 1.15, 1.01, 0.91, 1.05, 1.15, 1.01, 0.91],
 ],
 citadelle:  [
  ...
  [1.16, 1.0, 0.9, 1.16, 1.0, 0.9, 1.16, 1.0, 0.9, 1.16, 1.0, 0.9],
  [1.02, 1.15, 0.89, 1.02, 1.15, 0.89, 1.02, 1.15, 0.89, 1.02, 1.15, 0.89],
  [1.17, 1.02, 0.87, 1.02, 1.17, 1.02, 0.87, 1.02, 1.17, 1.02, 0.87, 1.02],
  [1.09, 1.15, 0.95, 0.89, 1.09, 1.15, 0.95, 0.89, 1.09, 1.15, 0.95, 0.89],
 ],
 nebuleuse:  [
  ...
  [1.17, 1.06, 0.87, 1.05, 1.06, 1.11, 0.87, 0.98, 1.17, 0.99, 0.98, 0.93],
  [0.94, 1.16, 1.06, 0.85, 1.06, 1.07, 1.1, 0.88, 0.98, 1.19, 0.98, 0.97],
  [0.96, 0.97, 1.13, 1.08, 0.85, 1.07, 1.08, 1.07, 0.91, 0.96, 1.2, 0.97],
  [0.95, 0.97, 0.97, 1.19, 0.99, 0.88, 1.09, 1.07, 1.07, 0.85, 1.05, 1.16],
 ],
 mandala:    [
  ...
  [1.13, 0.94, 0.94, 1.13, 0.94, 0.94, 1.13, 0.94, 0.93, 1.13, 0.94, 0.93],
  [1.12, 1.12, 1.0, 0.88, 0.88, 1.0, 1.12, 1.12, 1.0, 0.88, 0.88, 1.0],
  [1.1, 0.9, 1.1, 0.9, 1.1, 0.9, 1.1, 0.9, 1.1, 0.9, 1.1, 0.9],
  [1.11, 1.04, 0.89, 0.96, 1.11, 1.04, 0.89, 0.96, 1.11, 1.04, 0.89, 0.96],
 ],
};
// Sélection de la variante — v12.4.44 (Lot 3, dette technique v19, ADR-86) :
// table fixe en priorité (garantit qu'aucune des 7 Odyssées connues ne
// partage plus jamais sa forme de base avec une autre, sur aucune région),
// avec repli sur l'ancien hash déterministe pour toute Odyssée future non
// encore répertoriée ici (compatibilité ascendante, aucun crash).
const _ISLAND_ODYSSEY_ORDER = ['prim', 'primfr', 'primhist', 'mat', 'matfr', 'col', 'colfr'];
function _islandVariantIdx(shape, advKey){
 const variants = _ISLAND_PROFILE_VARIANTS[shape] || _ISLAND_PROFILE_VARIANTS.colline;
 const fixedIdx = _ISLAND_ODYSSEY_ORDER.indexOf(advKey);
 if(fixedIdx !== -1 && fixedIdx < variants.length) return fixedIdx;
